...
public class ListaParadas {
    ...
    public static ArrayList<String> estacionesOrdenadas() {
        ArrayList<String> sortedstations = new ArrayList<String>();
        HashMap<String, Estacion>[] lines = getLineas();

        for (int i = 0; i < 3; i++) {
            for (Map.Entry<String, Estacion> entry : lines[i].entrySet()) {
                if (!sortedstations.contains(entry.getValue().getNombre()))
                    sortedstations.add(entry.getValue().getNombre());
            }
        }
    ...
    public static Estacion getEstacion(String origen) {
        HashMap<String, Estacion>[] lines = getLineas();

        for (int i = 0; i < 3; i++) {
            if (lines[i].containsKey(origen))
                return lines[i].get(origen);
        }

        return null;
    }

    public static HashMap<String, Estacion>[] getLineas() {
        HashMap<String, Estacion>[] lines = new HashMap[3];
        lines[0] = LUzbekistan;
        lines[1] = LChilonzor;
        lines[2] = LYunusabad;
